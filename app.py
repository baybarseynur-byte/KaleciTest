...
# --- 1. SİSTEM AYARLARI VE TÜRKÇE FONT DESTEĞİ ---
st.set_page_config(page_title="GKD Akademik Performans", layout="wide")

def font_yukle():
    """Türkçe karakter desteği için fontu sisteme kaydeder."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    font_isimlari = ["arial.ttf", "Arial.ttf", "ARIAL.TTF"]
    
    for name in font_isimlari:
        path = os.path.join(current_dir, name)
        if os.path.exists(path):
            try:
                # Arial_Tr adıyla fontu kaydediyoruz
                pdfmetrics.registerFont(TTFont('Arial_Tr', path))
                return 'Arial_Tr'
            except:
                continue
    
    st.error("HATA: arial.ttf dosyası bulunamadı! Türkçe karakterler düzgün çıkmayabilir.")
    return 'Helvetica'
...
# --- 3. TÜRKÇE KARAKTER UYUMLU PDF ÜRETME ---
def profesyonel_pdf_uret(secilen, analiz_datalari):
    ...
    # TÜM STİLLERİ TÜRKÇE FONT İLE GÜNCELLE
    ...
    akis.append(Paragraph("<b>Test Skorları ve Grup Karşılaştırması</b>", alt_baslik_stili))
    tablo_verisi = [[
        Paragraph("<b>Test Adı</b>", tablo_icerik_stili), 
        Paragraph("<b>Skor</b>", tablo_icerik_stili), 
        Paragraph("<b>Grup Ort.</b>", tablo_icerik_stili), 
        Paragraph("<b>Z-Skor</b>", tablo_icerik_stili), 
        Paragraph("<b>En İyi</b>", tablo_icerik_stili), 
        Paragraph("<b>En Kötü</b>", tablo_icerik_stili)
    ]]
    ...
    for r in analiz_datalari:
        tablo_verisi.append([
            ...
            Paragraph(str(r['En İyi']), tablo_icerik_stili), 
            Paragraph(str(r['En Kötü']), tablo_icerik_stili)
        ])
    ...
    akis.append(Spacer(1, 30))

    # Test Grafikleri
    for r in analiz_datalari:
        test_paketi = []
        test_paketi.append(Paragraph(f"• {r['Test']} Analizi", test_baslik_stili))
        test_paketi.append(Spacer(1, 10))
        
        # Matplotlib grafikleri için font ayarı (Streamlit varsayılanı genelde destekler ama garantiye alalım)
        plt.rcParams['font.family'] = 'sans-serif'
        plt.figure(figsize=(6, 3))
        kategoriler = ['En Kötü', 'Ortalama', 'Sporcu', 'En İyi']
        degerler = [float(r['En Kötü']), float(r['Grup Ort.']), float(r['Skor']), float(r['En İyi'])]
        renkler = ['#ff8a80', '#cfd8dc', '#1a237e', '#81c784']
        
        plt.barh(kategoriler, degerler, color=renkler)
        plt.title(f"{r['Test']} Kıyaslama", fontsize=10)
        plt.tight_layout()
        ...
        test_img = Image(img_buf, width=380, height=190)
        test_paketi.append(test_img)
...
with st.form("istasyon_form"):
    ...
    kaydet = st.form_submit_button("VERİLERİ SİSTEME İŞLE")

if kaydet:
    q = get_age_quarter(dogum)
    packet = {"Ad": ad, "Soyad": soyad, "Dogum_Tarihi": dogum, "Boy": boy, "Kilo": kilo, "Ayak": ayak, "El": el, "Ceyrek": q}
    ...
    st.success("Veriler kaydedildi. Sayfa yenileniyor...")
    st.rerun()

# --- 5. ANALİZ VE PDF İNDİRME ---
if secilen_profil is not None:
    ...
    if analiz_datalari:
        ...
        pdf_dosya = profesyonel_pdf_uret(secilen_profil, analiz_datalari)
        st.download_button("📄 Türkçe Karakter Uyumlu PDF Raporu İndir", pdf_dosya, f"{ad}_{soyad}_Analiz.pdf")# --- 3. PROFESYONEL PDF ÜRETME FONKSİYONU ---
def profesyonel_pdf_uret(secilen, analiz_datalari):
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4, rightMargin=50, leftMargin=50, topMargin=50, bottomMargin=50)
    styles = getSampleStyleSheet()
    ...
    # Özet İstatistik Tablosu
    akis.append(Paragraph("<b>Test Skorları ve Grup Karşılaştırması</b>", styles['Heading3']))
    tablo_verisi = [["Test Adı", "Skor", "Grup Ort.", "Z-Skor", "En İyi", "En Kötü"]]
    
    for r in analiz_datalari:
        tablo_verisi.append([r['Test'], r['Skor'], r['Grup Ort.'], r['Z-Skor'], r['En İyi'], r['En Kötü']])
...
if secilen_profil is not None:
    ...
    if analiz_datalari:
        st.table(pd.DataFrame(analiz_datalari))
        pdf_dosya = profesyonel_pdf_uret(secilen_profil, analiz_datalari)
        st.download_button("📄 Profesyonel PDF Raporu İndir", pdf_dosya, f"{ad}_{soyad}_Analiz.pdf")
