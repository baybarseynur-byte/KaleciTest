...
def veri_oku():
    if os.path.exists(DB_FILE):
        try:
            # Farklı encoding ve delimiter denemeleri ile hata payını azaltıyoruz
            try:
...
def veri_kaydet_ve_merge(yeni_df):
    mevcut = veri_oku()
    # Yeni verinin sütunlarını temizle
    ...
    for index, row in yeni_df.iterrows():
        ...
        if not ad or not soyad: continue
...
with st.sidebar:
    ...
    if not db.empty:
        ...
        if 'Ad' in db.columns and 'Soyad' in db.columns:
            ...
            st.warning("Veritabanı sütunları uyumsuz. Lütfen yeni kayıt yapın veya dosya yükleyin.")
    ...
    if yuklenen_dosya and st.button("Verileri Havuza Aktar"):
        try:
            ...
            df_yeni.columns = df_yeni.columns.str.strip() # Sütun boşluklarını temizle
...
if secilen_profil is not None:
    # Analiz ve tablo kısmı (Aynı kaldı)
    # ... (Yukarıdaki analiz mantığı devam ediyor)
    st.divider()
    f_db = veri_oku()
    ...
    for t_ad, mod in test_specs.items():
        if t_ad in secilen_profil:
            ...
            seri = akranlar[t_ad].replace(0, np.nan).dropna() if t_ad in akranlar else pd.Series()
